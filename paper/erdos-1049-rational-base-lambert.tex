...
computable constant he did not name.  What is supplied here is the rational
specialisation of the 2004 forms, with the denominator accounting and the
limit passage carried out in full, which identifies the printed
\(\mu\) as an admissible constant for \(F\) itself, and with it the strip
...
of the bases involved.  A companion Archimedean cap shows that a base-independent coefficient-height bound
on final pairs in \(\mathbb Z[X]^{2}\), together with a degree upper bound of order
\(\delta n^{2}\), yields only a sufficient cutoff \(\sigma/(\sigma+\delta)\le 1/2\).
Excluding a particular family beyond that cutoff requires its actual degree and remainder
asymptotics; the displayed degree estimate does not by itself rule out every construction
at bases with \(b^{2}\ge a\), \(3/2\) included.  Laurent polynomials may appear only
...
coefficient, and a Vandermonde determinant proves equality.  The equality
determines the first nonzero formal term; a bound for the residual at a fixed
value of \(q\) is a separate analytic statement.  A companion ordinary-proof
note records a positive discrete measure for these same rows, from which
\(V_N^*>0\) and \(\log(V_N^*/(C_N q^{B_N}))=O_q(N)\).  That estimate is not
used in the rational-base argument, and it does not prove irrationality at
\(3/2\).  This is a route closure, not an irrationality result:
...
V\"a\"an\"anen~\cite{bv1994}, which is the explicit antecedent compared here.  The present rational
specialisation has the larger displayed cutoff $\theta^{*}$; bibliographical
priority is not asserted.  Zudilin's later
...
No claim of priority is made for the analysis below.  The principal theorems
are ordinary proofs.  Their finite arithmetic and generic formal consumers
...
\]
Their printed hypothesis for $\alpha=-1$ is $\lambda<(1/2+1/\pi^{2})^{-1}$ with
$\lambda=\log h(q)/\log|q|$; at $q=a/b$ this reads
$\log a/\log(a/b)<(1/2+1/\pi^{2})^{-1}$, which is the displayed inequality.
...
printing a proof.  The theorem below specialises the 2004 forms at a rational
base and carries out the denominator accounting, which identifies the printed
$\mu=C_1/C_0$ of the 2004 direction as an admissible $c$ for $F$ itself.  It is
an explicit-constant extension of an announced remark, and what it buys
...
forms.
The remainder series associated to the source displays (8)--(11) is written with
final $q$-Pochhammer length $\beta-a_2$, as required by the gamma expression (7)
and the residues (8).  One unnumbered product display on p.~156 of the 2004 source
has the shorter length $\beta-a_2-1$; that indexing discrepancy is not used here.
...
$[1/14,1/12)$ and the last $[13/14,14/15)$.  The direction, the thirteen
demi-intervals, the values $C_1=545.5$ and $C_0=221.30008816\ldots$ and the
ratio $C_1/C_0=2.46497868\ldots$ are printed at \cite[p.~162]{zudilin2004},
where that ratio is the irrationality-exponent bound of
\cite[Thm.~1, p.~154]{zudilin2004} for integer bases.  The further digits
...
\noindent The open-closed difference $(1/\mu_{BV},\theta^*)$ gained over
...
base-uniform rank-two family in $\mathbb Z[X]^2$ whose degree is controlled by
the displayed upper bound, Zudilin's included; an actual-degree argument is
needed before excluding a particular family beyond that cutoff.  The coefficient-height hypothesis is recorded in
...
transform, and Gauss's lemma for division by the monic $\Omega_n$.  Its
hypotheses are discharged at the direction fixed above: the parameter tuple is
$c=(13,14,12,14,15,13)$ with $m(c)=15$ and $s(c)=n$, so $s(c)>0$, and the
source's~(14) holds because $12n+1\le14n+1$ and $26n+2\le27n+2\le28n+2$.  The
lemma therefore applies for every $n\ge1$.  The pair
...
\path{state/public-source-redacted://erdos1049_contour_adversary/}.
...
that its methods do not sharpen Zudilin~2004.  So none of the printed regions
for $F$ compared here reaches $\log4/\log31$.  What is claimed here is the
rational specialisation of the 2004 forms, which identifies the printed $\mu$
as an admissible constant for $F$ itself, together with the resulting decidable
membership test.  The theorem should be presented as proved, and no priority
...
decay faster through an unrecorded increase in formal $q$-order.  The same two formulas are the weight of the unique least-order Gram tuple $k_i=i$.  Formal
order does not by itself bound the residual at a fixed $q$; the companion
note \texttt{ErdosProblems/Erdos1049/ZudilinHankelPositiveMeasure.md} supplies a
positive discrete measure and the two-sided estimate
$\log(V_N^*/(C_N q^{B_N}))=O_q(N)$.  Neither statement extracts
a denominator factor, improves the remaining height balance, or proves
...
The serialized Lean replay and exact ranks $1$ through $7$ pass; only the filtered initial-monomial iteration for rows $j\ge3$ and all columns remains formal.
...
For nonzero integer scalar factors and positive compared heights, integer scalar
multiplication leaves the ratio of a certified divisor to the matching absolute height
unchanged.  The underlying homogeneous scaling identities continue to hold at scalar
zero, but that ratio need not be defined there.  Under the endpoint hypotheses of
Proposition~\ref{res:commonmult}, a common divisor of the unscaled evaluations
\(H_W(U)\) and \(H_W(V)\) is coprime to \(6\).
\end{corollary}

\begin{proof}
Assume the scalar factors are nonzero and the compared heights are positive, so the
divisor/height ratio is defined.  By Theorem~\ref{res:content} integer scalar content
multiplies the analytic error and the exterior determinant, including the absolute
determinant height, by exactly the factors it introduces, so a divisor obtained that
way is paid for by the same factor in that height.  By Proposition~\ref{res:commonmult}
an integer dividing both specialised evaluations is divisible by neither \(2\) nor \(3\).
\end{proof}
